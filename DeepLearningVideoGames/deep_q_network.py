...
def createNetwork():
    ...
    # input layer
    s = tf.placeholder("float", [None, WORLD_SIZE, WORLD_SIZE, 4])
    ...
    h_conv3_flat = tf.reshape(h_conv3, [-1, WTF])
...
def trainNetwork(s, readout, h_fc1, sess):
    ...
    x_t = cv2.cvtColor(cv2.resize(x_t, (WORLD_SIZE, WORLD_SIZE)), cv2.COLOR_BGR2GRAY)
    ...
    while "pigs" != "fly":
        ...
        for i in range(0, K):
            ...
            x_t1 = cv2.cvtColor(cv2.resize(x_t1_col, (WORLD_SIZE, WORLD_SIZE)), cv2.COLOR_BGR2GRAY)
            ret, x_t1 = cv2.threshold(x_t1,1,255,cv2.THRESH_BINARY)
            x_t1 = np.reshape(x_t1, (WORLD_SIZE, WORLD_SIZE, 1))
